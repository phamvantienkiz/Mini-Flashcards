...
- **Project name**: Mini Flashcards
- **Stack**: Next.js, TypeScript, Tailwind CSS, FastAPI, SQLite (chuyển sang PostgreSQL ở phase sau)
- **Current goal**: Thiết lập dự án, phát triển ứng dụng flashcard local first với UI/UX đẹp mắt
- **Active branch**: feat/requirement-project
- **Key docs**: (Chưa có) Sẽ cập nhật khi có các file docs cụ thể như Skeleton Project.md, note.md
- **Run command**: (Sẽ cập nhật sau khi setup framework)
- **Test command**: (Sẽ cập nhật sau khi setup framework)
- **Build command**: (Sẽ cập nhật sau khi setup framework)
- **Log locations**: (Sẽ cập nhật sau)
